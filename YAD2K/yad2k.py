...
def _main(args):
    ...
    assert weights_path.endswith(
        '.weights'
    ), f'{weights_path} is not a .weights file'


    output_path = os.path.expanduser(args.output_path)
    assert output_path.endswith(
        '.h5'
    ), f'output path {output_path} is not a .h5 file'
        ...
        for section in cfg_parser.sections():
            ...
            if section.startswith('convolutional'):
                ...
                if activation == 'leaky':
                    ...
                    raise ValueError(
                        f'Unknown activation function `{activation}` in section {section}'
                    )
                ...
                if cfg_parser.items(section) != []:
                    raise ValueError(f'{section} with params unsupported.')
                ...
                not section.startswith('net')
                and not section.startswith('cost')
                and not section.startswith('softmax')
            ):
